...
def parse_literal(bin_str):
    ...
    ans += check[1:]
    return int(ans,2),nchunks,bin_str

def parse_bitlimit():
    pass

def parse_packetlimit():
    pass

tot_version = 0
for packet in bin_input:
    packet = '00111000000000000110111101000101001010010001001000000000'
    while packet != len(packet)*'0':
        version = packet[0:3]
        tot_version += int(version,2)
        type_id = packet[3:6]
        if type_id == '100':
            val,nchunks,remainder = parse_literal(packet[6:])
            packet = remainder
        else:
            len_type = packet[6]
            if len_type == '0':
                bit_len = int(packet[7:7+15],2)
                val,nchunks,bin_str = parse_literal(packet[22:22+bit_len])
            else:
