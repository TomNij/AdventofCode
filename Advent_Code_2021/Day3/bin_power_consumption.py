...
for bit in range(n_bits):
    if len(ox_gen)> 1:
        if return_median_at_pos(ox_gen,bit) == 0:
            ox_gen = [line for line in ox_gen if line[bit] == 0]
        else:
            ox_gen = [line for line in ox_gen if line[bit] == 1]
    if len(co2_scrub) > 1:
        if return_median_at_pos(co2_scrub,bit) == 0:
            # median == 0, least common value == 1
            co2_scrub = [line for line in co2_scrub if line[bit] == 1]
        else: #tie,take 0
            co2_scrub = [line for line in co2_scrub if line[bit] == 0]
